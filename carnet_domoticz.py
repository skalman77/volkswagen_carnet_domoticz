...
class VWCarnet(object):
	...
	# Update Domotiz device with Battery level
	BATTERY_LEVEL = vehicle_data['emanager']['EManager']['rbc']['status']['batteryPercentage']
	requests.get('http://' + DOMOTICZ_SERVER + '/json.htm?type=command&param=udevice&idx=' + DOM_BATTERY_LEVEL_VALUE + '&nvalue=0&svalue=' + str(BATTERY_LEVEL))

	# Update Domoticz device with Battery range
	BATTERY_RANGE = vehicle_data['emanager']['EManager']['rbc']['status']['electricRange']
	requests.get('http://' + DOMOTICZ_SERVER + '/json.htm?type=command&param=udevice&idx=' + DOM_RANGE_VALUE + '&nvalue=0&svalue=' + str(BATTERY_RANGE))
